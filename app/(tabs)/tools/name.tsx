...
function ResultCard({ number, label, meaning, colors }: { number: number; label: string; meaning: { title: string; description: string }; colors: MaterialColors }) {
  return (
    <View style={{ backgroundColor: colors.surfaceVariant, borderRadius: 12, padding: 14, marginBottom: 10 }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={{ fontSize: 11, fontWeight: '600', color: colors.onSurfaceVariant, textTransform: 'uppercase', letterSpacing: 1 }}>{label}</Text>
          <Text style={{ fontSize: 16, fontWeight: '700', color: colors.onSurface }}>{meaning.title}</Text>
        </View>
      </View>
      <Text style={{ fontSize: 14, color: colors.onSurface, marginTop: 10, lineHeight: 20 }}>{meaning.description}</Text>
    </View>
  );
}
...
export default function NameAnalysisScreen() {
  ...
  const results = useMemo(() => {
    ...
    return {
      expression: { number: expr, title: exprData?.expression?.title ?? '', description: exprData?.expression?.lifePurpose ?? '' },
      soulUrge: { number: soul, title: soulData?.soulUrge?.title ?? '', description: soulData?.soulUrge?.innerDesires ?? '' },
      personality: { number: pers, title: persData?.personality?.title ?? '', description: persData?.personality?.externalImage ?? '' },
    };
  }, [name]);
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: colors.background }} contentContainerStyle={{ padding: 16, paddingBottom: 40 }}>
      ...
      {results && (
        <>
          <ResultCard number={results.expression.number} label="Expression Number" meaning={{ title: results.expression.title, description: results.expression.description }} colors={colors} />
          <ResultCard number={results.soulUrge.number} label="Soul Urge Number" meaning={{ title: results.soulUrge.title, description: results.soulUrge.description }} colors={colors} />
          <ResultCard number={results.personality.number} label="Personality Number" meaning={{ title: results.personality.title, description: results.personality.description }} colors={colors} />
        </>
      )}
    </ScrollView>
  );
